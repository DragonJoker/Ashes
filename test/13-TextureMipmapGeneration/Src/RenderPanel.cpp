...
namespace vkapp
{
	...
	void RenderPanel::doCreateTexture()
	{
		...
		auto texLevel = tex2D[0u];

		if ( auto extent = VkExtent3D{ uint32_t( texLevel.extent().x ), uint32_t( texLevel.extent().y ), 1u };
			ashes::checkExtent( format, extent ) )
		{
			views.emplace_back( m_texture->createView( VK_IMAGE_VIEW_TYPE_2D
				, format
				, 0u ) );
			auto staging = std::make_unique< ashes::StagingBuffer >( m_device->getDevice()
				, 0u
				, texLevel.size() );
