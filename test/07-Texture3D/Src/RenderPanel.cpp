...
namespace vkapp
{
	namespace
	{
		...
		int constexpr TimerTimeMs = 20;
		VkFormat constexpr DepthFormat = VK_FORMAT_D32_SFLOAT;
	}
		...
		, m_timer{ this, int( Ids::RenderTimer ) }
	{
		for ( auto i = 0u; i < 100u; ++i )
		{
			...
			m_offscreenIndexData.push_back( uint16_t( index + 0 ) );
			m_offscreenIndexData.push_back( uint16_t( index + 1 ) );
			m_offscreenIndexData.push_back( uint16_t( index + 2 ) );
			m_offscreenIndexData.push_back( uint16_t( index + 2 ) );
			m_offscreenIndexData.push_back( uint16_t( index + 1 ) );
			m_offscreenIndexData.push_back( uint16_t( index + 3 ) );
	...
	void RenderPanel::doUpdateProjection()
	{
		...
		m_projectionMtx = utils::Mat4{ m_device->getDevice().perspective( float( utils::toRadians( 90.0_degrees ) )
			, width / height
			, 0.01f
			, 100.0f ) };
	...
	void RenderPanel::doUpdate()
	{
		...
		m_rotate = utils::rotate( m_rotate
			...
			, { 0, 1, 0 } );
		m_objectMtx = originalTranslate * m_rotate;
		m_stagingBuffer->uploadUniformData( *m_graphicsQueue
			, *m_commandPool
			, &m_objectMtx
			, 1u
