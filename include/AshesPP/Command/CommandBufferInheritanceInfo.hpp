...
namespace ashes
{
	...
	*/
	struct VkCommandBufferInheritanceInfo
	{
		...
		*	If this value includes the QueryControlFlag::ePrecise bit, then the active query can return boolean results or actual sample counts.
		*	If this bit is not set, then the active query must not use the QueryControlFlag::ePrecise bit.
		*\~french
		*\brief
		...
		*	Si cette valeur contient QueryControlFlag::ePrecise, alors la requête active peut renvoyer des résultats booléens ou le compte des échantillons.
		*	Sinon la requête active ne doit pas utiliser QueryControlFlag::ePrecise.
		*/
		VkQueryControlFlags queryFlags;
		/**
		*\~english
		*\brief
		*	A bitmask of QueryPipelineStatisticFlag specifying the set of pipeline statistics that can be counted by an active query in the primary command buffer when this secondary command buffer is executed.
		...
		*\~french
		*\brief
		*	Un masque binaire de QueryPipelineStatisticFlag définissant l'ensemble de statistiques de pipeline pouvant être comptées dans le tampon de commandes primaire lorsque celui-ci est exécuté.
