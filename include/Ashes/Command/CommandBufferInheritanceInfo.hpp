...
*/
#ifndef ___Ashes_CommandBufferInheritanceInfo_HPP___
#define ___Ashes_CommandBufferInheritanceInfo_HPP___
#pragma once

#include "Ashes/Image/ImageSubresourceLayers.hpp"
...
namespace ashes
{
	...
	*/
	struct CommandBufferInheritanceInfo
	{
		...
		*	If this value includes the ashes::QueryControlFlag::ePrecise bit, then the active query can return boolean results or actual sample counts.
		*	If this bit is not set, then the active query must not use the ashes::QueryControlFlag::ePrecise bit.
		*\~french
		*\brief
		...
		*	Si cette valeur contient ashes::QueryControlFlag::ePrecise, alors la requête active peut renvoyer des résultats booléens ou le compte des échantillons.
		*	Sinon la requête active ne doit pas utiliser ashes::QueryControlFlag::ePrecise.
		*/
		QueryControlFlags queryFlags;
		/**
		*\~english
		*\brief
		*	A bitmask of ashes::QueryPipelineStatisticFlag specifying the set of pipeline statistics that can be counted by an active query in the primary command buffer when this secondary command buffer is executed.
		...
		*\~french
		*\brief
		*	Un masque binaire de ashes::QueryPipelineStatisticFlag définissant l'ensemble de statistiques de pipeline pouvant être comptées dans le tampon de commandes primaire lorsque celui-ci est exécuté.
