...
*/
#ifndef ___AshesPP_CommandBufferInheritanceInfo_HPP___
...
namespace ashes
{
	...
	//	*	If this value includes the VK_QUERY_CONTROL_PRECISE_BIT bit, then the active query can return boolean results or actual sample counts.
	//	*	If this bit is not set, then the active query must not use the VK_QUERY_CONTROL_PRECISE_BIT bit.
	//	*\~french
	//	*\brief
	...
	//	*	Si cette valeur contient VK_QUERY_CONTROL_PRECISE_BIT, alors la requête active peut renvoyer des résultats booléens ou le compte des échantillons.
	//	*	Sinon la requête active ne doit pas utiliser VK_QUERY_CONTROL_PRECISE_BIT.
	//	*/
	//	VkQueryControlFlags queryFlags;
	//	/**
	//	*\~english
	//	*\brief
	//	*	A bitmask of VkQueryPipelineStatisticFlagBits specifying the set of pipeline statistics that can be counted by an active query in the primary command buffer when this secondary command buffer is executed.
	...
	//	*\~french
	//	*\brief
	//	*	Un masque binaire de VkQueryPipelineStatisticFlagBits définissant l'ensemble de statistiques de pipeline pouvant être comptées dans le tampon de commandes primaire lorsque celui-ci est exécuté.
