...
namespace gl_renderer
{
		...
		, bool dynamicDepthBias )
	{
		...
		if ( state != save )
		{
				...
				|| state.frontFace != save.frontFace )
			{
				...
				{
					if ( save.cullMode == VkCullModeFlagBits::eNone )
					{
						glLogCall( context
							, glEnable
							, GL_CULL_FACE );
				...
				else if ( save.cullMode != VkCullModeFlagBits::eNone )
				{
					glLogCall( context
						, glDisable
						, GL_CULL_FACE );
	...
	void BindPipelineCommand::apply( ContextLock const & context )const
	{
		...
		if ( !m_dynamicViewport )
		{
			...
			gl_renderer::apply( m_device, context, m_pipeline.getViewports() );
		}

		if ( !m_dynamicScissor )
		{
			assert( m_pipeline.hasScissor() );
			gl_renderer::apply( m_device, context, m_pipeline.getScissors() );
		}
