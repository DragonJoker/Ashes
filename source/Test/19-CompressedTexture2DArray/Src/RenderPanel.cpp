...
namespace vkapp
{
	...
	{
		m_uniformBuffer = ashes::makeUniformBuffer< LayerSelect >( *m_device
			, 1u
			, 0u
	...
	void RenderPanel::doCreateTexture()
	{
		...
		m_view = m_texture->createView( ashes::TextureViewType::e2DArray
			, format
			, 0u
			, uint32_t( tex2DArray.levels() )
			, 0u
	...
	void RenderPanel::doCreatePipeline()
	{
		...
		m_pipeline = m_pipelineLayout->createPipeline( ashes::GraphicsPipelineCreateInfo
		{
			...
			*m_renderPass,
			...
			{ ashes::DynamicState::eScissor, ashes::DynamicState::eViewport }
		} );
	}
