...
extern "C"
{
	...
	VkBool32 ashIsUsingICD()
	{
#if !defined( ASHES_ICD )
		return VK_TRUE;
#else
		return g_library.isUsingICD ? VK_TRUE : VK_FALSE;
#endif
	}

#if ASHES_ICD

	Ashes_API PFN_vkVoidFunction VKAPI_PTR vk_icdGetInstanceProcAddr( VkInstance instance
		, const char * name )
	{
		g_library.isUsingICD = true;
	...
	Ashes_API PFN_vkVoidFunction VKAPI_PTR vk_icdGetPhysicalDeviceProcAddr( VkInstance instance
		, const char * name )
	{
		g_library.isUsingICD = true;
	...
	Ashes_API VkResult VKAPI_PTR vk_icdNegotiateLoaderICDInterfaceVersion( uint32_t * pVersion )
	{
		g_library.isUsingICD = true;
		auto result = g_library.init();
