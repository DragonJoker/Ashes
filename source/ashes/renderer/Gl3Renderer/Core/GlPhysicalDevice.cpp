...
#define GL_VIEW_CLASS_BPTC_FLOAT                           0x82D3
		...
		, VkImageFormatProperties & imageProperties )const
	{
		...
		GLint value;
		glGetInternalformativ( gltype, internal, GL_INTERNALFORMAT_SUPPORTED, 1, &value );
		...
		glGetInternalformativ( gltype, internal, GL_MAX_WIDTH, 1, &value );
		...
		{
			glGetInternalformativ( GL_TEXTURE_2D, internal, GL_MAX_HEIGHT, 1, &value );
			imageProperties.maxExtent.height = uint32_t( value );

			if ( type == VK_IMAGE_TYPE_3D )
			{
				glGetInternalformativ( GL_TEXTURE_3D, internal, GL_MAX_DEPTH, 1, &value );
				imageProperties.maxExtent.depth = uint32_t( value );
			}
		}

		glGetInternalformativ( gltype, internal, GL_SAMPLES, 1, &value );
		imageProperties.sampleCounts = VkSampleCountFlagBits( value );

		glGetInternalformativ( gltype, internal, GL_IMAGE_TEXEL_SIZE, 1, &value );
		VkDeviceSize texelSize = VkDeviceSize( value );

		gltype = convert( type, 2u, 0u );
		glGetInternalformativ( gltype, internal, GL_MAX_LAYERS, 1, &value );
