...
namespace ashes::D3D11_NAMESPACE
{
		...
		, m_createInfo{ std::move( createInfo ) }
	{
		...
		desc.AddressU = spl::convert( createInfo.addressModeU );
		desc.AddressV = spl::convert( createInfo.addressModeV );
		desc.AddressW = spl::convert( createInfo.addressModeW );
		spl::convert( createInfo.borderColor, desc.BorderColor );
		desc.ComparisonFunc = getComparisonFunc( createInfo.compareOp );
		desc.Filter = spl::convert( createInfo.minFilter
