...
namespace d3d11_renderer
{
	namespace
	{
		ID3D11ShaderResourceView * getView( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
		...
		ID3D11ShaderResourceView * getBufferView( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
			assert( index < write.texelBufferView.size() );
			return static_cast< BufferView const & >( write.texelBufferView[index].get() ).getView();
		}
		
		ID3D11UnorderedAccessView * getImageUAV( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
			assert( index < write.imageInfo.size() );
			return static_cast< TextureView const & >( write.imageInfo[index].imageView.value().get() ).getUnorderedAccessView();
		}

		ID3D11SamplerState * getSampler( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
			assert( index < write.imageInfo.size() );
			return static_cast< Sampler const & >( write.imageInfo[index].sampler.value().get() ).getSampler();
		}

		ID3D11Buffer * getBuffer( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
			assert( index < write.bufferInfo.size() );
			return static_cast< Buffer const & >( write.bufferInfo[index].buffer.get() ).getBuffer();
		}

		ID3D11UnorderedAccessView * getBufferUAV( renderer::WriteDescriptorSet const & write, uint32_t index )
		{
			assert( index < write.imageInfo.size() );
			return nullptr;// static_cast< Buffer const & >( write.bufferInfo[index].buffer.get() ).getUnorderedAccessView();
		}

		template< renderer::ShaderStageFlag Flag >
		struct Binder;

		template<>
		struct Binder< renderer::ShaderStageFlag::eCompute >
		{
		...
		template<>
		struct Binder< renderer::ShaderStageFlag::eVertex >
		{
			static void bind( ID3D11DeviceContext1 * context, UINT bindingIndex, ID3D11ShaderResourceView * image, ID3D11SamplerState * sampler )
			{
				bind( context, bindingIndex, image );
		...
		template<>
		struct Binder< renderer::ShaderStageFlag::eGeometry >
		{
			static void bind( ID3D11DeviceContext1 * context, UINT bindingIndex, ID3D11ShaderResourceView * image, ID3D11SamplerState * sampler )
			{
				bind( context, bindingIndex, image );
		...
		template<>
		struct Binder< renderer::ShaderStageFlag::eTessellationControl >
		{
			static void bind( ID3D11DeviceContext1 * context, UINT bindingIndex, ID3D11ShaderResourceView * image, ID3D11SamplerState * sampler )
			{
				bind( context, bindingIndex, image );
		...
		template<>
		struct Binder< renderer::ShaderStageFlag::eTessellationEvaluation >
		{
			static void bind( ID3D11DeviceContext1 * context, UINT bindingIndex, ID3D11ShaderResourceView * image, ID3D11SamplerState * sampler )
			{
				bind( context, bindingIndex, image );
		...
		template<>
		struct Binder< renderer::ShaderStageFlag::eFragment >
		{
			static void bind( ID3D11DeviceContext1 * context, UINT bindingIndex, ID3D11ShaderResourceView * image, ID3D11SamplerState * sampler )
			{
				bind( context, bindingIndex, image );
		...
		template< renderer::ShaderStageFlag Flag, typename ... Params >
		void tryBindOne( ID3D11DeviceContext1 * context
			, UINT bindingIndex
			, renderer::ShaderStageFlags const & flags
			, Params ... params )
		{
		...
		void tryBind( ID3D11DeviceContext1 * context
			...
			, Params ... params )
		{
			tryBindOne< renderer::ShaderStageFlag::eCompute >( context
				, bindingIndex
				, flags
				, params... );
			tryBindOne< renderer::ShaderStageFlag::eVertex >( context
				, bindingIndex
				, flags
				, params... );
			tryBindOne< renderer::ShaderStageFlag::eGeometry >( context
				, bindingIndex
				, flags
				, params... );
			tryBindOne< renderer::ShaderStageFlag::eTessellationControl >( context
				, bindingIndex
				, flags
				, params... );
			tryBindOne< renderer::ShaderStageFlag::eTessellationEvaluation >( context
				, bindingIndex
				, flags
				, params... );
			tryBindOne< renderer::ShaderStageFlag::eFragment >( context
				, bindingIndex
				, flags
				, params... );
		}
