...
namespace d3d11_renderer
{
	...
	void PushConstantsCommand::apply( Context const & context )const
	{
		...
		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eVertex ) )
		{
			context.context->VSSetConstantBuffers( m_pcb.getLocation(), 1u, &buffer );
		}

		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eTessellationControl ) )
		{
			context.context->HSSetConstantBuffers( m_pcb.getLocation(), 1u, &buffer );
		}

		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eTessellationEvaluation ) )
		{
			context.context->DSSetConstantBuffers( m_pcb.getLocation(), 1u, &buffer );
		}

		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eGeometry ) )
		{
			context.context->GSSetConstantBuffers( m_pcb.getLocation(), 1u, &buffer );
		}

		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eFragment ) )
		{
			context.context->PSSetConstantBuffers( m_pcb.getLocation(), 1u, &buffer );
		}

		if ( checkFlag( m_pcb.getStageFlags(), renderer::ShaderStageFlag::eCompute ) )
		{
