...
namespace d3d11_renderer
{
	...
	void ShaderModule::loadShader( std::string const & shader )
	{
		...
		if ( m_source.find( "rendererScalePosition" ) != std::string::npos )
		{
			m_source = R"(float4 rendererScalePosition(float4 pos)
{
	float4x4 scale;
	...
	void ShaderModule::compile( renderer::ShaderStageState const & state )
	{
		static std::map< renderer::ShaderStageFlag, std::string > Profiles
		{
			{ renderer::ShaderStageFlag::eVertex, "vs_5_0" },
			{ renderer::ShaderStageFlag::eGeometry, "gs_5_0" },
			{ renderer::ShaderStageFlag::eTessellationControl, "hs_5_0" },
			{ renderer::ShaderStageFlag::eTessellationEvaluation, "ds_5_0" },
			{ renderer::ShaderStageFlag::eFragment, "ps_5_0" },
			{ renderer::ShaderStageFlag::eCompute, "cs_5_0" },
		};
		...
		{
			renderer::Logger::logError( reinterpret_cast< char * >( errors->GetBufferPointer() ) );
			renderer::Logger::logError( m_source );
		}
