...
namespace d3d11_renderer
{
	namespace
	{
		D3D11_QUERY convert( renderer::QueryType type )
		{
			switch ( type )
			{
	...
	void QueryPool::getResults( uint32_t firstQuery
		...
		, renderer::UInt32Array & datas )const
	{
		...
		for ( auto i = firstQuery; i < max; ++i )
		{
			...
			while ( hr == S_FALSE || !checkFlag( flags, renderer::QueryResultFlag::eWait ) );
	...
	void QueryPool::getResults( uint32_t firstQuery
		...
		, uint32_t stride
		, renderer::QueryResultFlags flags
		, renderer::UInt64Array & datas )const
	{
		auto max = firstQuery + queryCount;
		assert( max <= m_queries.size() );
		ID3D11DeviceContext * context;
