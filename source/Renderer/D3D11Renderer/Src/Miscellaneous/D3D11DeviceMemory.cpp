...
namespace d3d11_renderer
{
	...
	private:
		renderer::BufferTargets m_bufferTargets;
	...
	private:
		void doInitTexDesc( renderer::ImageCreateInfo const & createInfo
			, D3D11_TEXTURE1D_DESC & desc )
		{
			...
			desc.BindFlags = ( ( checkFlag( m_usage, renderer::ImageUsageFlag::eSampled )
			...
			desc.MiscFlags = ( isRenderTarget( m_usage )
					...
					: 0 )
				| ( checkFlag( createInfo.flags, renderer::ImageCreateFlag::eCubeCompatible )
			...
			, D3D11_TEXTURE2D_DESC & desc )
		{
			...
			desc.BindFlags = ( ( checkFlag( m_usage, renderer::ImageUsageFlag::eSampled )
				...
				| ( isStorage( m_usage )
					? D3D11_BIND_UNORDERED_ACCESS
					: 0 ) );
			desc.MiscFlags = ( ( isRenderTarget( m_usage ) && checkFlag( m_usage, renderer::ImageUsageFlag::eSampled ) )
					? D3D11_RESOURCE_MISC_GENERATE_MIPS
					: 0 )
				| ( checkFlag( createInfo.flags, renderer::ImageCreateFlag::eCubeCompatible )
					? D3D11_RESOURCE_MISC_TEXTURECUBE
	...
	ID3D11Buffer * DeviceMemory::bindToBuffer( renderer::BufferTargets targets )
	{
	...
	ID3D11Texture1D * DeviceMemory::bindToTexture1D( renderer::ImageCreateInfo const & createInfo )
	{
	...
	ID3D11Texture2D * DeviceMemory::bindToTexture2D( renderer::ImageCreateInfo const & createInfo )
	{
	...
	ID3D11Texture3D * DeviceMemory::bindToTexture3D( renderer::ImageCreateInfo const & createInfo )
	{
