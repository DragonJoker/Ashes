...
namespace gl_renderer
{
	...
	void BindPipelineCommand::apply()const
	{
		...
		doApply( m_device, m_pipeline.getColourBlendState() );
		doApply( m_device, m_pipeline.getRasterisationState() );
		doApply( m_device, m_pipeline.getDepthStencilState() );
		doApply( m_device, m_pipeline.getMultisampleState() );
		doApply( m_device, m_pipeline.getTessellationState() );

		if ( m_pipeline.hasViewport() )
		{
			doApply( m_device, m_pipeline.getViewport() );
		}

		if ( m_pipeline.hasScissor() )
		{
			doApply( m_device, m_pipeline.getScissor() );
		}
