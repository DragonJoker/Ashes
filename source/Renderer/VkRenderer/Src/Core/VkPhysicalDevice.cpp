...
namespace vk_renderer
{
		...
		, m_renderer{ renderer }
	{
		...
		auto res = m_renderer.EnumerateDeviceExtensionProperties( m_gpu
			, nullptr
			, &extensionCount
			, nullptr );
		...
		res = m_renderer.EnumerateDeviceExtensionProperties( m_gpu
			, nullptr
			, &extensionCount
			, extensions.data() );
		...
		m_renderer.GetPhysicalDeviceMemoryProperties( m_gpu, &m_memoryProperties );
		m_renderer.GetPhysicalDeviceProperties( m_gpu, &m_properties );
		m_renderer.GetPhysicalDeviceFeatures( m_gpu, &m_features );

		// Et enfin les propriétés des familles de files du GPU.
		uint32_t queueCount{ 0 };
		m_renderer.GetPhysicalDeviceQueueFamilyProperties( m_gpu, &queueCount, nullptr );
		assert( queueCount >= 1 );

		m_queueProperties.resize( queueCount );
		m_renderer.GetPhysicalDeviceQueueFamilyProperties( m_gpu, &queueCount, m_queueProperties.data() );
		assert( queueCount >= 1 );
	}
