...
namespace vk_renderer
{
	...
	{
		m_device.GetDeviceQueue( m_device, familyIndex, 0, &m_queue );
	}
		...
		, renderer::Fence const * fence )const
	{
		...
		auto res = m_device.QueueSubmit( m_queue
			, 1u
			, &submitInfo
		...
		, SemaphoreCRefArray const & semaphoresToWait )const
	{
		...
		return m_device.QueuePresentKHR( m_queue, &presentInfo );
	}

	bool Queue::waitIdle()const
	{
		auto res = m_device.QueueWaitIdle( m_queue );
		return checkError( res );
	}
}
