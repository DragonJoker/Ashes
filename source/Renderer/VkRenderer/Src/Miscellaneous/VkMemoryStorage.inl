...
namespace vk_renderer
{
		...
		: m_device{ device }
	{
		...
		auto res = m_device.AllocateMemory( m_device, &allocateInfo, nullptr, &m_memory );
	...
	{
		m_device.FreeMemory( m_device, m_memory, nullptr );
	}
		...
		, VkMemoryMapFlags flags )const
	{
		...
		auto res = m_device.MapMemory( m_device
			, m_memory
			, offset
			, size
			, 0u/*flags*/
		...
		, bool modified )const
	{
		if ( modified )
		{
			...
			auto res = m_device.FlushMappedMemoryRanges( m_device, 1, &flushRange );
		...
		m_device.UnmapMemory( m_device, m_memory );
	}
